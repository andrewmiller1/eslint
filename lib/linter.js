 ...
 * @property {("disable"|"enable"|"disable-line"|"disable-next-line"|"disable-next-effective-line")} type
 ...
 * @param {("disable"|"enable"|"disable-line"|"disable-next-line"|"disable-next-effective-line")} type The type of directive comment
...
function getDirectiveComments(filename, ast, ruleMapper) {
    ...
    ast.comments.filter(token => token.type !== "Shebang").forEach(comment => {
        ...
        const match = /^(eslint(-\w+){0,4}|exported|globals?)(\s|$)/.exec(trimmedCommentText);
        ...
        if (/^eslint-disable-(next-)?(next-effective-)?line$/.test(match[1])) {
